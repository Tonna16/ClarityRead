...
(() => {
  ...
  // replace setInterval handshake cleanup with chrome.alarms (MV3-friendly)
  chrome.alarms.create('clarity_handshake_cleanup', { periodInMinutes: 1 });

chrome.alarms.onAlarm.addListener((alarm) => {
  try {
    if (alarm && alarm.name === 'clarity_handshake_cleanup') {
      try {
  ...
  } catch (e) { safeWarn('alarms.onAlarm handler failed', e); }
});
