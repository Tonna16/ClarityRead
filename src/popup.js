        ...
        const runner = (payload) => {
          try {
            ...
            const speak = (text, opts) => {
              ...
              if (opts && typeof opts.rate !== 'undefined') u.rate = clampRate(opts.rate);
              if (opts && typeof opts.pitch !== 'undefined') u.pitch = Number(opts.pitch) || 1;
            ...
            if (payload.action === 'speedRead') {
              ...
              let i = 0;
              const clampRate = (r) => Math.max(0.5, Math.min(1.6, Number(r) || 1));
              const speakChunk = () => {
            ...
            if (payload.action === 'stopReading') {
              try { window.speechSynthesis.cancel(); } catch (e) {}
              return { ok: true };
            }
            if (payload.action === 'pauseReading') {
              try { window.speechSynthesis.pause(); } catch (e) {}
              return { ok: true };
            }
            if (payload.action === 'resumeReading') {
        ...
        try {
          chrome.scripting.executeScript({
            ...
            if (chrome.runtime.lastError) {
              console.warn('fallback runner executeScript failed:', chrome.runtime.lastError.message);
          ...
          console.error('fallbackExecuteRunner threw:', ex);
          return resolve({ ok: false, error: 'runner-exception', detail: String(ex) });
        }
      };

      // get last-focused window's active tab first (avoids picking the popup tab)
      try {
        chrome.windows.getLastFocused({ populate: true }, (lastWin) => {
          let candidateTab = null;
          if (lastWin && Array.isArray(lastWin.tabs)) {
            candidateTab = lastWin.tabs.find(t => t.active && t.id && !isUnsupportedUrl(t.url));
          }
          if (candidateTab) {
            // send to that tab
            console.info('using lastFocusedWindow active tab', { id: candidateTab.id, url: candidateTab.url });
            chrome.tabs.sendMessage(candidateTab.id, message, (response) => {
              ...
              console.warn('popup: initial sendMessage error to lastFocusedWindow tab:', errMsg);
              // if "Receiving end does not exist", attempt injection then fallback runner
              if (errMsg && errMsg.includes('Receiving end does not exist')) {
                chrome.scripting.executeScript({ target: { tabId: candidateTab.id }, files: ['src/contentScript.js'] }, (execRes) => {
                  if (chrome.runtime.lastError) {
                    ...
                    return fallbackRunner(candidateTab.id, message);
                  }
                  chrome.scripting.insertCSS({ target: { tabId: candidateTab.id }, files: ['src/inject.css'] }, () => {
                    chrome.tabs.sendMessage(candidateTab.id, message, (resp2) => {
                      ...
                      console.warn('popup: sendMessage after injection still failed:', err2);
                      if (err2 && err2.includes('Cannot access contents of the page')) return resolve({ ok: false, error: 'no-host-permission', detail: err2 });
                      return fallbackRunner(candidateTab.id, message);
                    });
                  });
                });
          ...
          // If no valid lastFocused tab found, fallback to scanning tabs (previous behavior)
          chrome.tabs.query({}, (allTabs) => {
            const candidate = (allTabs || []).find(t => !isUnsupportedUrl(t.url));
            if (!candidate) {
              console.warn('No usable web tab found to send message to.');
              return resolve({ ok: false, error: 'no-tab' });
            }
            console.info('Active tab missing or unsupported; using candidate tab', { id: candidate.id, url: candidate.url });
            // proceed using the same send flow as above
            chrome.tabs.sendMessage(candidate.id, message, (response) => {
              if (!chrome.runtime.lastError) return resolve({ ok: true, response });
              const errMsg = chrome.runtime.lastError && chrome.runtime.lastError.message;
              console.warn('popup: initial sendMessage error to candidate tab:', errMsg);
              if (errMsg && errMsg.includes('Receiving end does not exist')) {
                chrome.scripting.executeScript({ target: { tabId: candidate.id }, files: ['src/contentScript.js'] }, (execRes) => {
                  if (chrome.runtime.lastError) {
                    const msgLow = (chrome.runtime.lastError.message || '').toLowerCase();
                    if (msgLow.includes('cannot access contents of the page') || msgLow.includes('must request permission')) {
                      return resolve({ ok: false, error: 'no-host-permission', detail: chrome.runtime.lastError.message });
                    }
                    return fallbackRunner(candidate.id, message);
                  }
                  chrome.scripting.insertCSS({ target: { tabId: candidate.id }, files: ['src/inject.css'] }, () => {
                    chrome.tabs.sendMessage(candidate.id, message, (resp2) => {
                      if (!chrome.runtime.lastError) return resolve({ ok: true, response: resp2 });
                      const err2 = chrome.runtime.lastError && chrome.runtime.lastError.message;
                      console.warn('popup: sendMessage after injection still failed:', err2);
                      if (err2 && err2.includes('Cannot access contents of the page')) return resolve({ ok: false, error: 'no-host-permission', detail: err2 });
        ...
        console.error('popup: sendMessageToActiveTabWithInject threw:', ex);
